...
2.  `hope.open.api.enable` 是否暴露 swagger 文档信息
3.  `hope.open.api.showMeta` 是否暴露 API meta 信息
...
所有这些 **资源** 信息都是由 `wire` 模块生成，非运行时代码反射扫描所得，API+API元信息均在设计阶段已经固定，运行时不能修改！

如需要修改，需要重新 编译&发布你的 `wire` 模块！

<TipInfo>建议在生产环境关闭此类信息接口</TipInfo>
