...
Based on `firstLanguage` & `secondLanguage` configurations in [ui](/docs/ui/003_app#configuration):
...
4. `i18key` is auto-formatted to start with `pages.` and end with `.title`, aligning with the framework’s i18n mechanism (see `pagePrefix` & `pageSuffix` in [ui](/zhCN-docs/ui/003_app#configuration)).
