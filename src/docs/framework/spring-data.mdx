...
In ApiHug, there are two ways to make an entity identifiable:

1. Use the `wires: [ALL]` or `wires: [IDENTIFIABLE]` configuration.
2. Define a custom **ID** column by marking a single column with `id: true` (exactly one such column per table).

You may choose **either** approach, but **not both**, to designate an entity as identifiable—this ensures proper **CRUD** handling by the repository.

When using a custom ID column—such as a user-defined primary key like a user ID—how do we determine whether the entity is new?

By default, if a new entity instance includes an ID, the repository treats it as an **update** operation, which can prevent insertion of new records. To address this, starting from **`1.4.4-RELEASE`**, ApiHug provides built-in support for more precise new-entity detection by integrating Spring Data’s `Persistable` interface.

ApiHug automatically wires helper logic into your entity to support this behavior:
...
Since such IDs are typically assigned by business logic from the outset, the `isNew()` method allows explicit control over whether the operation should be treated as an **insert** or **update**, ensuring correct persistence behavior.
