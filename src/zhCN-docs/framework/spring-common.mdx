export const title = "ApiHug Spring Common 扩展";
export const description =
  "ApiHug SDK spring common 扩展文档和设计原理";


## Spring Common

Spring 扩展包， 包含基本共享类和函数，为更快更安全的开发：[Maven Central](https://search.maven.org/artifact/com.apihug/it-common-spring)
...
**checker** 包定义了表达 Authority 与 Role 组合的 DSL，并不只是“编译期标记”。运行时会加载生成的权限元数据，将其映射到 `SecurityContext`，再由 `HopeSecurityManager` 在当前 service 方法上解析并执行校验。

<TipInfo>checker DSL 会和生成的 wire 元数据一起生效。先在 proto 中定义 authorization，再由 Spring 运行时通过 `SecurityContext` 和 `HopeSecurityManager` 解析并执行。</TipInfo>
