...
💁 如果你的项目没有使用 `ApiHug` 的领域实体管理能力，这次迁移对你的影响通常会比较有限。

Latest: [Maven Central](https://search.maven.org/artifact/com.apihug/it-bom)

`0.7.8-RELEASE` 主要引入了：

- 把 Repository `@Derived` 的维护位置拆分到独立的 `trait` 目录
...
2. 把派生 repository 逻辑移动到更容易维护的位置

可以用模块 `book-app` 中现有的 repository `com.novel.book.wire.domain.book.repository.BookAuthorRepository` 作为示例。
...
⚠️ 在执行 `stub` 命令前，请先备份你的 `@Derived` 方法。

这次调整之后，`stub` 目录会被视为纯生成代码，因此下一轮 `stub` 运行时可以被整体覆盖。
...
然后执行模块对应的 `stub` 命令。具体命令一般可以在项目的 `README.md` 里找到：

```shell
./gradlew.bat book-app:clean stub build -x test -x stubTest
```

命令执行成功后，你会看到一个新的 `sourceSet`：`book-app\src\main\trait`。

💡 你可能需要在 `Gradle` 工具窗口中执行 `Reload Gradle Project`，否则 `book-app\src\main\trait` 可能不会被识别为 `sourceSet`。

### 第二步

1. 把备份下来的 `@Derived` 方法移动到 `trait` 目录里的 `_BookAuthorRepository`，并在首次迁移阶段临时保持旧 repository 仍然可编译。
2. 再次执行 `stub` 命令。

之后，所有 `@Derived` repository 方法都会维护在 `book-app\src\main\trait` 下。

1. `stub` 任务会把这些方法重新合并回运行时的 `BookAuthorRepository`。
2. 生成模板代码和自定义派生逻辑被分开后，`BookAuthorRepository` 会更干净。
...
如果 repository 数量可控，例如少于 30 个，这种方式仍然是可接受的。
...
从 SDK `0.8.5-RELEASE` 开始，后续版本对这一模式做了更容易维护的改进。
...
按照 IDEA 的建议操作后，方法会被提升到父接口里，从而减少一次额外的 `stub` 运行。
...
也可以参考 Scala 中的 [Companion objects](https://zhCN-docs.scala-lang.org/scala3/book/domain-modeling-tools.html#companion-objects)。
